...
[![Version](https://img.shields.io/badge/version-2.24.3-f97316)](CHANGELOG.md)
...
## 🆕 Quoi de neuf en 2.24.3

- 📖 **Centre d'aide vérifié et actualisé** : revue complète des 33
  sujets — ajout de l'exigence https pour la galerie mobile et de la
  portée exacte du verrou de confidentialité (Corbeille et vignettes
  comprises).

## 🆕 Quoi de neuf en 2.24.2

- ⚡ **Optimisations** : vues Corbeille et Masquées sur index dédiés
  (coût indépendant de la taille de la bibliothèque), et détection de
  voyages plus rapide — les groupes d'une même zone partagent un seul
  appel de géocodage au lieu d'un chacun.

## 🆕 Quoi de neuf en 2.24.1

- ✅ **Zéro vulnérabilité, point final** : montée du bundler
  (electron-vite 5, Vite 7) qui élimine les 3 dernières alertes — elles
  ne touchaient que le serveur de développement. `npm audit` intégral :
  0 vulnérabilité, production comme développement. L'audit de sécurité
  est entièrement soldé.

## 🆕 Quoi de neuf en 2.24.0

- ⚡ **Electron 33 → 42** (Chromium 148, Node 24) : clôt le dernier point
  de l'audit de sécurité — la série 33 en fin de vie cumulait 5 advisories
  haute sévérité. Montée groupée de l'outillage (better-sqlite3 12,
  electron-builder 26, exiftool-vendored 37). 0 vulnérabilité en
  production, toute la batterie de tests repasse à l'identique.

## 🆕 Quoi de neuf en 2.23.4

- 🛡️ **Fin des correctifs d'audit** : sandbox Chromium actif sur la
  fenêtre principale, https obligatoire pour la synchronisation web (le
  jeton ne circule plus jamais en clair, localhost excepté), et vues
  Corbeille/Masquées paginées comme le reste de l'app.
...
- 🛡️ **Dépendance sécurisée** : mise à niveau d'`exiftool-vendored`
  (lecture EXIF) pour corriger une vulnérabilité haute sévérité
  (GHSA-cw26-7653-2rp5). `npm audit` : 0 vulnérabilité. Extraction EXIF
  revérifiée à l'identique sur photos réelles (dates + GPS).

## 🆕 Quoi de neuf en 2.23.1

- 🔒 **Correctifs d'audit** : la Corbeille respecte le verrou de
  confidentialité (une photo masquée en corbeille n'est plus visible sans
  mot de passe), les miniatures et proxys vidéo sont vraiment supprimés
  du disque à la suppression définitive, le GPS corrompu (0,0) ne fausse
  plus la détection de voyages, et une fuite de listeners a été colmatée.
...
- 🧳 **Regroupement voyages / événements** : sélectionne des photos, clique
  sur 🧳 Voyages dans la barre du tray, ajuste l'écart maximum (1-72h), puis
  détecte automatiquement les groupes de photos prises en rafale. Chaque
...
- 🗑 **Corbeille** : mettre à la corbeille (réversible, ↩ Annuler) au lieu
  de supprimer directement, vue dédiée dans la barre latérale, restaurer,
  et suppression définitive (confirmation requise) qui efface réellement
  le fichier du disque. Vérifié de bout en bout avec de vrais fichiers.

## 🆕 Quoi de neuf en 2.20.3

- 🎞️ **Diaporama plus fluide** : chaque calque anime désormais son propre
  zoom/panoramique en continu, y compris pendant le fondu — le fondu
  redevient un pur effet d'opacité, sans plus jamais interrompre le
  mouvement.

## 🆕 Quoi de neuf en 2.20.2

- 🎞️ **Diaporama** : la photo qui s'estompe ne saute plus vers une autre
  animation juste avant de disparaître dans le fondu.

## 🆕 Quoi de neuf en 2.20.1

- 🍎 **Script d'auto-correction macOS** (`Corriger PicaLibre si besoin.command`,
  dans les assets de chaque release) : double-clic après une mise à jour
  si Gatekeeper bloque le lancement — plus besoin du Terminal.
