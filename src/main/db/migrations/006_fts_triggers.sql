...
-- content='photos_search_content' avec triggers de synchronisation automatique.

-- 1. Supprimer l'ancienne table FTS (vide, jamais utilisée)
--    FTS5 crée des shadow tables nommées d'après la table virtuelle:
--    photos_fts_data, photos_fts_idx, photos_fts_config, photos_fts_content
--    Il faut toutes les nettoyer pour éviter "table already exists"
...
-- 4. Triggers de synchronisation photos_search_content → photos_fts
--    Ces triggers se déclenchent sur toute modification de
--    photos_search_content et mettent à jour l'index FTS en conséquence.
...
-- 5. Triggers sur photos → photos_search_content

-- INSERT
CREATE TRIGGER photos_fts_ai AFTER INSERT ON photos BEGIN
  INSERT INTO photos_search_content (rowid, caption, filename, tags, persons)
  SELECT
...
CREATE TRIGGER photos_fts_au AFTER UPDATE OF filename, caption ON photos BEGIN
  UPDATE photos_search_content SET caption = new.caption, filename = new.filename WHERE rowid = new.id;
END;
...
-- 10. Peuplement initial : remplir photos_search_content avec les données existantes
