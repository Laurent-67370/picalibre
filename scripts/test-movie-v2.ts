...
async function main() {
  ...
  await sharp({ create: { width: 1200, height: 900, channels: 3, background: { r: 200, g: 60, b: 40 } } })
    .jpeg().toFile('/tmp/mv-a.jpg')
  await sharp({ create: { width: 900, height: 1200, channels: 3, background: { r: 40, g: 120, b: 210 } } })
    .jpeg().toFile('/tmp/mv-b.jpg')
  // Vidéo 3 s AVEC audio (mire + sinus)
  spawnSync(FF, ['-y', '-f', 'lavfi', '-i', 'testsrc=size=640x480:rate=30:duration=3',
    '-f', 'lavfi', '-i', 'sine=frequency=440:duration=3',
    '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '/tmp/mv-clip.mp4'])
  // Deux pistes audio de 4 s et 3 s
  spawnSync(FF, ['-y', '-f', 'lavfi', '-i', 'sine=frequency=300:duration=4', '-c:a', 'aac', '/tmp/mv-t1.m4a'])
  spawnSync(FF, ['-y', '-f', 'lavfi', '-i', 'sine=frequency=600:duration=3', '-c:a', 'aac', '/tmp/mv-t2.m4a'])

  assert.ok(Math.abs((await probeDuration(FF, '/tmp/mv-clip.mp4')) - 3) < 0.15, 'probeDuration')
  console.log('✅ probeDuration (parse ffmpeg -i)')

  const stackA = upsertOp(emptyStack(), { type: 'temperature', params: { value: 0.6 } })
  const items = [
    { filepath: '/tmp/mv-a.jpg', stack: stackA },
    { filepath: '/tmp/mv-clip.mp4', stack: emptyStack(), isVideo: true },
    { filepath: '/tmp/mv-b.jpg', stack: emptyStack() }
  ]

  // --- 1. Photos + vidéo, fondu, SANS bande-son (audio du clip conservé) ---
  const r1 = await makeMovie(items, {
    ffmpegPath: FF, preset: 'ultrafast', width: 480, height: 270, durationSec: 2, transition: 'fade', outFile: '/tmp/mv-out1.mp4'
  })
  assert.equal(r1.segments, 3)
  assert.ok(Math.abs(r1.totalDuration - 7) < 0.2, `durée attendue ~7s, calculée ${r1.totalDuration}`)
  const d1 = await probeDuration(FF, '/tmp/mv-out1.mp4')
  assert.ok(Math.abs(d1 - 7) < 0.3, `durée réelle ~7s, mesurée ${d1}`)
  const s1 = streams('/tmp/mv-out1.mp4')
  ...
  const r2 = await makeMovie(items, {
    ...
    audioPaths: ['/tmp/mv-t1.m4a', '/tmp/mv-t2.m4a'], outFile: '/tmp/mv-out2.mp4'
  })
  const d2 = await probeDuration(FF, '/tmp/mv-out2.mp4')
  assert.ok(Math.abs(d2 - r2.totalDuration) < 0.3, `bornée au film : ${d2} vs ${r2.totalDuration}`)
  assert.ok(streams('/tmp/mv-out2.mp4').hasAudio, 'bande-son présente')
  ...
  const r3 = await makeMovie(
    [items[0], items[2]],
    { ffmpegPath: FF, preset: 'ultrafast', width: 480, height: 270, durationSec: 2, transition: 'none', outFile: '/tmp/mv-out3.mp4' }
  )
  const d3 = await probeDuration(FF, '/tmp/mv-out3.mp4')
  ...
  await makeMovie([items[0]], {
    ffmpegPath: FF, preset: 'ultrafast', width: 480, height: 270, durationSec: 2, audioPath: '/tmp/mv-t2.m4a', outFile: '/tmp/mv-out4.mp4'
  })
  assert.ok(streams('/tmp/mv-out4.mp4').hasAudio, 'compat audioPath v1')
